...
module Integral
  module Widgets
    describe RecentPosts do
      let(:posts) { Integral::Post.published.limit(2).order(published_at: :desc) }
      let(:render_options) { {
      ...
      describe '.render' do
        ...
        context 'amount option' do
          context 'when amount is passed in' do
            let(:posts) { Integral::Post.published.limit(3).order(published_at: :desc) }

            it 'renders with set amount of posts' do
              expect(Integral::ApplicationController).to receive(:render).with(render_options)
        ...
        context 'tagged option' do
          context 'when tagged is passed in' do
            let(:posts) { Integral::Post.published.tagged_with('foo bar'.split).limit(2).order(published_at: :desc) }
